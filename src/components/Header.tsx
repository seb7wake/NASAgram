...
const Header: React.FC<HeaderProps> = (props) => {
  ...
  const handleSelect = (ranges:any) => {
    ranges.selection.endDate = ranges.selection.endDate > maxDate ? maxDate : ranges.selection.endDate
    selectionRange = {
    ...
    return (
      showCalendar ? 
        <div style={{marginBottom: '2em'}}>
          <button className='liked-posts-btn' style={{display:'flex', marginBottom: '1em'}} onClick={() => props.setShowLiked(!props.showLiked)}>View Liked Posts</button>
          <div style={{
          ...
          >
            ...
            />
            <button className='exit-btn' onClick={() => setShowCalendar(!showCalendar)}></button>
          </div> 
        </div>
        : 
        getFilterButton()
  ...
  const getFilterButton = () => {
    return (
      <div style={{display:'flex', marginBottom: '2em'}}>
        <button className='liked-posts-btn' onClick={() => props.setShowLiked(!props.showLiked)}>View Liked Posts</button>
        <button className='pill-btn' onClick={() => setShowCalendar(!showCalendar)}>
